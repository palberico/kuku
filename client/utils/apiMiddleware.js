...
const tokenMiddleware = args => store => next => action => {
  if (!action)
    action = { type: '' }
  let { customHeaders = [], validateAction = 'VALIDATE_TOKEN',  logoutAction = 'LOGGED_OUT', axios } = args;
  HEADERS = [...new Set([...HEADERS, ...customHeaders])]
  if (action.type === validateAction) {
    HEADERS.forEach( async (token) => {
      axios.defaults.headers.common[token] = await AsyncStorage.getItem(token)
    });
